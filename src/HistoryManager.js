...
class HistoryManager {
  ...
  */
  constructor() {
    this.listeners = [];
    this.audioCacheService = new AudioCacheService();
    this.loadHistory();
  }
  ...
  */
  async saveToHistory(notes, analysisResponse, audioBlob = null) {
    ...
    let audioId = null;
    if (audioBlob) {
      audioId = await this.audioCacheService.saveAudioToCache(
        audioBlob, 
        analysisId, 
        notes
      );
  ...
  async loadAudioForHistoryItem(historyItem) {
    if (!historyItem.audioId) return null;
    return await this.audioCacheService.loadAudioFromCache(historyItem.audioId);
  }
