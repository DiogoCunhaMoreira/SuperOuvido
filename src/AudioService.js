class AudioService {
    ...
    */
    async processAudioBlob(blob, setIsAnalyzing, setWarningInfo, setStatus, setProgress, noteDetector, setActiveNotes, setDetectedNotes) {
      try {
        setIsAnalyzing(true);
        setWarningInfo("");
    ...
    async startRecording(setStatus, setIsRecording, setRecordingDuration, setActiveNotes, setDetectedNotes, setRecordingComplete, setRecordedAudio, setWarningInfo) {
      try {
        ...
        */
        this.recorder.addEventListener("stop", () => {
          ...
          setStatus(`Gravação completa (${((Date.now() - this.recordingStartTime) / 1000).toFixed(1)}s)`);
        });
